...
import { fetchProposals } from '../../lib/queries';
...
export default function MyProposals() {
  ...
  const { data: proposals = [], isLoading } = useQuery({
    queryKey: QK.proposals(currentUser?.id ?? ''),
    queryFn:  () => fetchProposals(currentUser!.id),
    enabled:  !!currentUser,
  });
        ...
        {proposals.length === 0 ? (
          ...
          <div className="space-y-4">
            {proposals.map((proposal) => (
              <div key={proposal.id} className="bg-white rounded-xl shadow-sm p-6 hover:shadow-md transition-shadow">
                    ...
                    <div className="flex items-center gap-3 mb-2 flex-wrap">
                      ...
                      {proposal.hasDiscussion && proposal.status === 'pending' && (
                        <span className="flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold bg-amber-100 text-amber-700 animate-pulse">
                          <MessageCircle className="w-3 h-3" /> Client wants to discuss
                        </span>
                      )}
                    </div>
                    <p className="text-gray-600 mb-3 line-clamp-2">{proposal.cover_letter}</p>
                  </div>
                  <div className="ml-4 text-green-600 font-bold text-lg">
                    ₦{proposal.proposed_amount.toLocaleString()}
                  </div>
                </div>
                <div className="flex gap-3">
                  <button onClick={() => navigate(`/job/${proposal.job_id}`)}
                    className="flex-1 bg-blue-950 text-white py-2 rounded-lg font-semibold hover:bg-blue-900 transition-colors">
                    View Job
                  </button>
                  {proposal.hasDiscussion && proposal.status === 'pending' && (
                    <button onClick={() => navigate(`/messages?autoJobId=${proposal.job_id}&proposalId=${proposal.id}&freelancerId=${proposal.freelancer_id}`)}
                  ...
                  {proposal.status === 'accepted' && proposal.job.status === 'in_progress' && (
                    <button onClick={() => navigate(`/messages?autoJobId=${proposal.job_id}`)}
